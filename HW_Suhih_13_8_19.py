...
Price=0 # вводим переменную для определения стоимости билета

i=1
while i<N+1: # заходим в цикл до указанного количества билетов
    try: # будем писать исключение по ошибке ввода возраста
        Price=int(input(f'Print an age of the person for {i} ticket')) # запрашиваем возраст посетителя
        if 18<=Price<=25: # указываем варианты возраста и рассчитываем стоимость каждого билета
            Sum+=990
